...
export default function WorkSpace() {
    ...
    const agents = [
        { title: "Chat GPT", icon: "/images/logos/chat-logo.png" },
        { title: "Google Bard", icon: "/images/icons/Google_Bard.png" },
        { title: "Elsa", icon: "/images/icons/Elsa.webp" }
    ];
    ...
    const [selectedAgent, setSelectedAgent] = useState('Chat GPT');

    return (
        <div>
            <div className="flex h-screen bg-gray-100">
                {/* options */}
                <div className="flex flex-col items-center w-16 h-screen py-8 space-y-8 bg-gray-900 dark:border-gray-700">
                    <a href="/">
                ...
                {/* sidebar */}
                <div className="w-1/5 py-4 bg-gray-800 text-white flex flex-col items-center">
                    {
                        agents.map((agent, i) => (
                            <div key={i} className="w-5/6 my-2 px-2 py-2 hover:bg-slate-700 rounded-xl flex justify-between items-center font-semibold text-sm cursor-pointer"
                                onClick={() => setSelectedAgent(agent.title)}
                            >
                                <div className="flex items-center">
                                    <img src={`${agent.icon}`} width={40} alt="" />
                                    <span className="pl-2 text-base">{selectedAgent === agent.title ? "New Chat" : agent.title}</span>
                                </div>
                                {selectedAgent === agent.title ? editIcon() : <></>}
                            </div>
                        ))
                    }

                    <p className="w-5/6 mt-12 px-2 text-slate-400 text-xs">Previous 7 days</p>

                    {/* chat history */}
                    <div className="w-full my-4 flex flex-col items-center">
                        {
                            chats.map((agent, i) => (
                                <div key={i} className="w-5/6 px-2 py-3 hover:bg-slate-700 rounded-xl text-base cursor-pointer">{agent}</div>
                            ))
                        }
                    </div>

                    <p className="w-5/6 mt-12 px-2 text-slate-400 text-xs">Previous 30 days</p>
